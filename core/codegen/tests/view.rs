...
//   assert_eq!(result, "<div><h1>Hello</h1><p>World</p></div>");
// }

#[test]
fn test_functional_component_with_attributes() {
  #[component]
  fn HelloWorld<'a>(title: &'a str, count: i32) {
    html! {
      <div>
        <h1>{title}</h1>
        <p>"Count:" {count}</p>
  ...
  let result: String = html! { <HelloWorld title="Hello" count={5} /> };
  assert_eq!(
    result,
    "<hello-world title=\"hello\" count=5><div><h1>Hello</h1><p>Count: 5</p></div></hello-world>"
  );
}
