...
use html::attributes::{Attribute, AttributeValue};
...
fn parse_tag_name<'a, E: ParseError<&'a str> + ContextError<&'a str>>(
  input: &'a str,
) -> IResult<&'a str, Ident, E> {
  let (input, name) = context(
    "parse_tag_name",
    take_while(|c: char| c.is_alphanum() || c == '-' || c == '_'),
  )(input)?;
  Ok((input, Ident::new(name, Span::call_site())))
}
  ...
  move |input: &'a str| {
    ...
    Ok((
      ...
      HtmlElement {
        ...
        children: Default::default(),
        name: Ident::new("meta", Span::call_site()),
      },
    ))
  }
}
...
fn parse_tag_with_children<'a, E: ParseError<&'a str> + ContextError<&'a str>>(
  ...
  if html_tag.name.to_string().is_empty() {
    Ok((
      input,
      HtmlFragment {
